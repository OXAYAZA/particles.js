( function () {
	...
	// Установка тэга прототипа (для будущего определения типа)
	...
	// Запуск симуляции
	ParticlesCanvas.prototype.run = function () {
	...
	// Установка свойств частицы случайным образом (скорость движения и положение)
	...
	// Обновление параметров частицы за один цикл
	...
	if ( !window.ParticlesCanvas ) {
		window.ParticlesCanvas = ParticlesCanvas;
	} else {
		throw new Error( 'ParticlesCanvas is already occupied' );
	}

	if ( !window.Particle ) {
		window.Particle = Particle;
	} else {
		throw new Error( 'Particle is already occupied' );
	}
})();
